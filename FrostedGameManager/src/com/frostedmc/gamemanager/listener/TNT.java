...
public class TNT implements Listener {
    ...
    public void onTNTUse(PlayerInteractEvent event) {
		try {
			if(event.getAction() == Action.RIGHT_CLICK_AIR && event.getItem().getType() == Material.TNT || event.getAction() == Action.RIGHT_CLICK_BLOCK && event.getItem().getType() == Material.TNT) {
		        ...
		        tnt.setMetadata("fakeTNT", new FixedMetadataValue(GameManager.getInstance(), true));
		        
		        if(ChatColor.stripColor(event.getItem().getItemMeta().getDisplayName()).equalsIgnoreCase("Super TNT")) {
		        	tnt.setMetadata("superTNT", new FixedMetadataValue(GameManager.getInstance(), true));
		        }
			}
		} catch (Exception e) {}
	}
    ...
    public void onEntityExplode(EntityExplodeEvent event) {
        ...
        if(GameManager.getInstance().getCurrentGame() != null
                && !GameManager.getInstance().getCurrentGame().gameFlags.enableExplosions) {
            for (Entity ent : event.getEntity().getNearbyEntities(5, 5, 5)) {
                if (ent instanceof Player) {
                    if(!SpectatorMode.getInstance().contains(((Player) ent))) {
                        ...
                        if(GameManager.getInstance().getCurrentGame().gameFlags.instaKillExplosions) {
                            if(event.getEntity() instanceof Projectile) {
                                if(((Projectile) event.getEntity()).getShooter() != null) {
                                    if(((Projectile) event.getEntity()).getShooter() instanceof Player) {
                                        if(!((Player) ((Projectile) event.getEntity()).getShooter()).getName().equals(ent.getName())) {
                                            DamageManager.handleDeath(((Player) ent), (Entity) ((Projectile) event.getEntity()).getShooter(),
                                                    "Projectile");
                                        }
                                    } else {
                                        DamageManager.handleDeath(((Player) ent), (Entity) ((Projectile) event.getEntity()).getShooter(),
                                    ...
                                    DamageManager.handleDeath(((Player) ent), event.getEntity(),
                                            DamageManager.formatCause(EntityDamageEvent.DamageCause.ENTITY_EXPLOSION));
                                }
                            }
                        }
                    }
            ...
            for(org.bukkit.block.Block block : Utils.getBlocksInRadius(event.getEntity().getLocation(), 4, false)) {
                if(block.getType().isSolid()) {
                    if(!block.isLiquid()) {
                        ...
                        fallingBlock.setMetadata("customExplosion", new FixedMetadataValue(GameManager.getInstance(), true));
        ...
        for(Map.Entry<org.bukkit.block.Block, Object[]> block : finalList.entrySet()) {
            ...
            }.runTaskLater(GameManager.getInstance(), 120+(2*new Random().nextInt(6)));
        }
    }
}